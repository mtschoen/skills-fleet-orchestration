...
Default: `acceptEdits`.
When using `claude`, the wrapper seeds an allowlist into the worktree's `.claude/settings.local.json` so the remote session can use Bash/Edit/Write/Read/Glob/Grep without prompts. The allowlist is narrow in tool count, not in power (see the caveat below). If a task needs additional permissions (e.g. `sudo` for system installs), pass `--extra-allow "Bash(sudo apt install *)"` to widen the allowlist for that specific run.
