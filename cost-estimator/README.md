...
A skill that analyzes cost and active time from local transcripts.
...
`AGENTS_COST_ROOTS="host-a:C:/Users/you/.claude/projects,host-b:Y:/.claude/projects"`
...
The analyzer prices *surviving* transcripts. Under Claude Code's default
...
retention time? Consider extending it to 365 days in order to track usage
...
invoking an agent on a task. Concrete shapes:
...
`ryoppippi/ccusage`(<https://github.com/ryoppippi/ccusage>) as an accuracy
check; that's the only third-party tool the skill body references.
