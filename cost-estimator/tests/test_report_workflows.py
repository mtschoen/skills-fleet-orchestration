...
def test_trend_data_reads_ranges_and_rejects_unknown_options(workspace_directory: Path):
    ...
    assert (
        trend_data.bucket_index(datetime(2026, 2, 1), datetime(2026, 1, 1), "month")
        == 1
    )
...
def test_plot_trend_helpers_render_escaped_html(load_script):
    ...
    rows = [
        {
            "_parsed_timestamp": datetime(2026, 3, 1),
            "_cost_usd_float": 1.23456,
            "label": "b",
        },
        {
            "_parsed_timestamp": datetime(2026, 3, 1),
        ...
        {
            "_parsed_timestamp": datetime(2026, 3, 2),
            "_cost_usd_float": 3.0,
            "label": "b",
        },
    ]
...
def test_plot_trend_main_writes_and_opens_chart(
    ...
    monkeypatch.setattr(
        ...
        [
            ...
            "week",
            "--csv",
            str(FIXTURES / "sessions-demo.csv"),
            "--out",
            str(output_path),
            "--open",
        ],
...
def test_plot_compare_helpers_cover_windows_buckets_and_rendering(load_script):
    ...
    parser = SimpleNamespace(
        error=lambda message: (_ for _ in ()).throw(ValueError(message))
    )
    month = SimpleNamespace(month="2026-03", start=None, end=None, last=None)
    start, end, mode, label, filename, span = plot_compare._resolve_current_window(
        month, parser
    )
    ...
    date_range = SimpleNamespace(
        month=None, start="2026-03-01", end="2026-03-02", last=None
    )
    ...
    assert plot_compare.aggregate_per_bucket(rows, datetime(2026, 3, 1), "day", 2) == [
        1.23,
        0.0,
    ]
    assert plot_compare.cumulative([1.23, 2.0]) == [1.23, 3.23]
    assert plot_compare.bucket_labels(start, datetime(2026, 2, 1), "day", 1)[
        0
    ].startswith("Day 1")
    assert plot_compare.bucket_labels(start, datetime(2026, 2, 1), "week", 1)[
        0
    ].startswith("Week 1")
    assert plot_compare.bucket_labels(start, datetime(2026, 2, 1), "month", 1)[
        0
    ].startswith("Month 1")
...
def test_plot_session_resolves_collects_renders_and_runs(
    ...
    assert plot_session.resolve_session("session-1", [projects]) == (
        parent,
        "session-123",
    )
...
def test_summarize_reports_paid_and_unpaid_views(
    monkeypatch, capsys, workspace_directory: Path
):
    csv_path = workspace_directory / "sessions.csv"
    ...
    monkeypatch.setattr(
        sys, "argv", ["summarize.py", "--csv", str(csv_path), "--paid", "20"]
    )
    summarize.main()
