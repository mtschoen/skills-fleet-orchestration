...
def _resolve_current_window(arguments, parser):
    ...
    if arguments.start:
        ...
        current_start, current_end = inclusive_date_bounds(
...
def main():
    ...
    (current_start, current_end, mode, range_label, range_filename, span_days) = (
        _resolve_current_window(arguments, parser)
    )
    ...
    print(
        f"Current window: {current_start.isoformat()} .. {current_end.isoformat()}",
        file=sys.stderr,
    )
    print(
    ...
    print(
        f"Bucket: {granularity} (span={span_days} days, mode={mode})", file=sys.stderr
    )
    ...
    print(
        f"Current rows: {len(current_rows)}  Prior rows: {len(prior_rows)}",
        file=sys.stderr,
    )
    ...
    current_per_bucket = aggregate_per_bucket(
        current_rows, current_start, granularity, total_buckets
    )
    prior_per_bucket = aggregate_per_bucket(
        prior_rows, prior_start, granularity, total_buckets
    )
