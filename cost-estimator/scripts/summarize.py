...
def main():
    ...
    print(
        "  Subagent processing time (reported separately; may overlap): "
    ...
    if commands_path.is_file():
        with open(commands_path, newline="", encoding="utf-8") as handle:
            ...
            for command_row in reader:
                ...
                command_row["active_seconds"] = float(
                    command_row["active_seconds"] or 0
                )
                command_rows.append(command_row)

    print("\n=== SLASH COMMAND TIME ===")
    command_totals = defaultdict(
        lambda: {"invocations": 0, "seconds": 0.0, "sessions": set()}
    )
    ...
    for command, bucket in sorted(
        ...
        print(
            f"  {command:30} {format_duration(bucket['seconds']):>12}  "
            f"{bucket['invocations']} timed invocations  "
            f"{len(bucket['sessions'])} sessions"
        )

    requested_command = (
        normalize_command(arguments.command) if arguments.command else None
    )
    ...
    for name, count in tool_totals.most_common(20):
        ...
        print(
            f"  {name:40}  {count:>6} calls   in {tool_sessions[name]:>3}/{len(rows)} sessions ({coverage:4.1f}%)"
        )

    # First-turn input bloat (MCP/skill loader proxy)
    print("\n=== FIRST-TURN INPUT TOKENS (system+tool-schema bloat per session) ===")
    sized = [
        (row["first_turn_input_tokens"], row)
        for row in rows
    ...
    if arguments.paid is not None and grand_total > 0:
        ...
        print(
            f"  Implied discount factor: {discount_factor:.4f} "
    ...
    if discount_factor is not None:
        print(
            f"  {'Raw $':>9}  {'Prorated $':>9}  {'Date':>10}  {'Hit':>5}  "
        ...
        for row in rows[:20]:
            ...
            print(
                f"  ${row['cost_usd']:>7.2f}  ${prorated:>7.2f}  {row['session_date']:>10}  "
        ...
        print(
            f"  {'Raw $':>9}  {'Date':>10}  {'Hit':>5}  "
            f"{'Sub':>4}  {'Turns':>5}  {'1stTurn':>9}  Top-5 tools"
        )
        for row in rows[:20]:
            first_turn_kib = row["first_turn_input_tokens"] // 1024
            print(
                f"  ${row['cost_usd']:>7.2f}  {row['session_date']:>10}  "
                f"{row['cache_hit_pct']:>4.1f}%  {row['subagent_count']:>4}  "
                f"{row['assistant_turns']:>5}  {first_turn_kib:>7}KT  "
                f"{row['top_tools'][:80]}"
            )
    ...
    if discount_factor is not None:
        print(
            f"  {'Date':>10}  {'Sessions':>8}  {'Raw $':>9}  {'Prorated $':>9}  "
